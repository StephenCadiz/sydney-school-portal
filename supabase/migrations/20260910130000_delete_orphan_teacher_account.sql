...
begin
  ...
  if v_duty_id is null
     or (select count(*) from public.friday_at_6_duties where teacher_id = p_teacher_id) <> 1
     or exists (select 1 from public.friday_at_6_duties where b1_teacher_id = p_teacher_id) then
