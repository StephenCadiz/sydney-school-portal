...
export default function TeacherStudentMonitoringTasks() {
  ...
  const [activeAction, setActiveAction] = useState<"feedback" | "continue" | null>(null);
  ...
  useEffect(() => {
    if (active) {
      ...
      const onKeyDown = (event: KeyboardEvent) => { if (event.key === "Escape" && !saving) { setActive(null); setActiveAction(null); } };
  ...
  return <>
    ...
    {active && <div className="teacher-monitoring-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) { setActive(null); setActiveAction(null); } }}><section className="teacher-monitoring-modal" role="dialog" aria-modal="true" aria-labelledby="teacher-monitoring-dialog-title"><button ref={closeButtonRef} className="teacher-monitoring-close" type="button" onClick={() => { if (!saving) { setActive(null); setActiveAction(null); } }} aria-label="Close">×</button><h2 id="teacher-monitoring-dialog-title">{active.student_name || "Student"} · {active.class_name}</h2><p>{active.level_name} · {active.reason}</p>{error && <p role="alert" className="error-message">{error}</p>}<div className="teacher-monitoring-form">{activeAction === "continue" ? <><p>The existing deadline of <strong>{active.feedback_due_on}</strong> will be preserved.</p><button type="button" disabled={saving} onClick={() => void submit("continue")}>{saving ? "Saving…" : "Continue monitoring"}</button></> : <><label>Observations<textarea required value={feedback.observations} onChange={(e) => setFeedback({ ...feedback, observations: e.target.value })} /></label><label>Strengths<textarea value={feedback.strengths} onChange={(e) => setFeedback({ ...feedback, strengths: e.target.value })} /></label><label>Difficulties<textarea value={feedback.difficulties} onChange={(e) => setFeedback({ ...feedback, difficulties: e.target.value })} /></label><label>Level suitability<textarea value={feedback.level_suitability} onChange={(e) => setFeedback({ ...feedback, level_suitability: e.target.value })} /></label><label>Recommendation<textarea value={feedback.recommendation} onChange={(e) => setFeedback({ ...feedback, recommendation: e.target.value })} /></label><button type="button" disabled={saving || !feedback.observations.trim()} onClick={() => void submit("feedback")}>{saving ? "Saving…" : "Submit feedback"}</button></>}</div></section></div>}
  </>;
}
