...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    await authorizeCambridgeAccess(actor, id);
    return NextResponse.json(await loadStudentAccess(id), { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (action === "save-email") {
      return NextResponse.json(await saveStudentEmail(id, normalizeStudentEmail(body?.email)), { headers: { "Cache-Control": "no-store" } });
    }
    if (action === "send-invitation" || action === "resend-invitation") {
      const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000";
      return NextResponse.json(await sendStudentInvitation(id, `${siteUrl.replace(/\/$/, "")}/set-password`, { resend: action === "resend-invitation" }), { headers: { "Cache-Control": "no-store" } });
    }
