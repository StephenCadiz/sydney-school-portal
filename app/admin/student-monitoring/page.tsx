    ...
    <section className="admin-page-card">
      ...
      <form onSubmit={createRecord} className="admin-student-monitoring-form">
        <div className="admin-monitoring-combobox-wrap admin-student-monitoring-wide">
          ...
          {matches.length > 0 && <ul id="monitoring-student-results" className="admin-monitoring-combobox-results" role="listbox">{matches.map((student) => <li key={`${student.student_type}:${student.id}`} role="option"><button type="button" onClick={() => void selectStudent(student)}><strong>{fullName(student)}</strong><span className="admin-monitoring-result-programme">Programme: {student.student_type === "young_learner" ? "Young Learner" : "Cambridge"} · {student.email || "No email"}</span>{student.enrolments?.length ? student.enrolments.length === 1 ? <span className="admin-monitoring-result-enrolment">{student.enrolments[0].level_name} · {student.enrolments[0].class_name} · {student.enrolments[0].days || "Days not set"} · {time(student.enrolments[0].start_time)}–{time(student.enrolments[0].end_time)} · {student.enrolments[0].teacher_name}</span> : <span className="admin-monitoring-result-enrolment">{student.enrolments.length} active enrolments · choose a class after selecting</span> : <span className="admin-monitoring-result-enrolment">No active enrolment</span>}</button></li>)}</ul>}
        </div>
